...
## Find the SUID files
echo -e "[ ! ] Running the command to find SUID files";
echo -e "[ * * ] find / -xdev -user root \( -perm -4000 -o -perm -2000 -o -perm -6000 \) 2>/dev/null";
...
if [ -z "$suidArray" ]
then
	echo -e "[ - ] The command has successfuly performed, but we did not find any related files";
	exit
fi

countsuidArray=$(echo $suidArray | tr " " "\n" | wc -l); # Count the output
echo -e "[ + ] The command has successfuly performed. We have found $countsuidArray file(s)";

echo -e "[ ! ] Let's compare the found SUID files with predefined base ($suidlistcount apps)";
...
then
	echo -e "[ - ] Unfortunately, there are no any SUID files, which lead to privilege escalation";
...
echo -e "[ + ] We have found at least $exploitablesuidarraycount potential SUID exploitable file(s):"
...
do
	...
	echo -e "\n[ * * ] $suidexploitable";
	echo -e "[ Explanation ] $suidexplanation";
done
...
do
	...
	do 
		if [ $suidcommand == $suidSelect ]
			...
			echo -e "[ * * ] We have found $suidSelect SUID file. Trying to get root";
...
if [ ! $wehavesuidtoroot ]
then echo -e "[ * * ] Seems like there are no privilege escalation files through SUID in the system. Follow the instructions above to read the restricted files, eg. /etc/shadow or /root/.bash_history, or perform other high privileges commands. P.s. think outside the box to pivot into the root ;)";
fi
